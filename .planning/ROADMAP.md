...
- [x] 20-02-PLAN.md — pure logic: silenced branch in decide_ambient_channel + decide_interaction_allowed predicate (OWNER-05/06)
- [x] 20-03-PLAN.md — GeminiService per-guild session counter + guild_id kwargs + non-events call-site threading (RATE-01)
...
> **Bitfield (D-09, user-confirmed):** TEN permissions, value `309240908864` — the 8-permission `3263552` from D-01's original table is SUPERSEDED (research found `/autolyrics`' thread call sites at `cogs/music.py:938,950,958,965`).
...
- [x] 23-04-PLAN.md — ci.yml site job (unprivileged scan) + pages.yml (workflow_run-gated deploy) + release.yml (multi-arch GHCR on `v*`)

**Wave 5** *(blocked on Wave 4)*

- [x] 23-05-PLAN.md — global.css design tokens + self-hosted mono + Layout/Hero/Cta/Footer (both invite CTAs)
