...
> Synced to the 7 plans authored 2026-07-14 (`63850e0`). Plan/Wave columns are now concrete;
> no row was dropped.

| Surface | Plan | Wave | Requirement | Threat Ref | Secure Behavior | Test Type | Automated Command | File Exists | Status |
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| Push Phases 19–22 to `origin/main`; CI green at HEAD | 23-01 | 1 | (D-13 prereq for PORT-03, CICD-02/03) | — | Four unexercised phases pass the existing gate before new CI surface is added | integration (real CI) | `gh run watch` on the push; `ruff format .` applied first (3 files known-red) | ✅ existing `ci.yml` | ⬜ pending |
| Astro site builds; `base: '/dexter'` correct | 23-03 | 3 | PORT-01 | — | N/A | integration | `cd site && npm ci && npm run build` exits 0; `dist/index.html` asset paths are `/dexter/…` | ❌ W0 | ⬜ pending |
| **`dist/*.html` invite-URL drift scan** | 23-03 (test) + 23-04 (CI wiring) | 3–4 | PORT-01, CICD-02 (regression) | T-23-DRIFT | A drifted/over-privileged OAuth2 URL in the shipped artifact **fails the build**; never silently skips in CI | unit + CI-required | `SITE_DIST_REQUIRED=1 pytest tests/test_site_drift_guard.py -v` | ❌ W0 | ⬜ pending |
| **Positive control** for the `dist/` scanner | 23-03 | 3 | PORT-01 (guard integrity) | T-23-DRIFT | Scanner provably finds a URL when one exists → a broken regex cannot green-vacuous-pass | unit | `pytest tests/test_site_drift_guard.py -k positive_control -v` | ❌ W0 | ⬜ pending |
| Existing invite drift guard stays green **and non-vacuous** | 23-03 | 3 | (Phase 22 regression) | T-23-DRIFT | Phase 22's guarantee survives the SSG migration | unit | `pytest tests/test_invite_drift_guard.py -v` | ✅ exists | ⬜ pending |
| Demo mock renders; transcript is **verbatim** Dexter output | 23-02 (source) + 23-06 (render) | 2, 6 | PORT-02 | T-23-HONEST | No roast line is authored by an executor; every line traces to the user-supplied source in `23-DEMO-TRANSCRIPT.md` | **manual-only** | N/A — code review + byte-diff against the human-supplied lines | ❌ W0 | ⬜ pending |
| README GIF rendered from the mock (one source, two derivatives) | 23-07 | 7 | PORT-02, PORT-03 | — | N/A | script + manual | `python scripts/render_demo_gif.py` produces a ≤2MB GIF; visual check | ❌ W0 | ⬜ pending |
| README case study: tagline, 5 badges, mermaid, callouts, invite link | 23-07 | 7 | PORT-03 | — | N/A | **manual-only** (GitHub's own markdown/mermaid renderer) | N/A — view the rendered README on github.com | ❌ W0 | ⬜ pending |
| CI badge points at the real `ci.yml` run and is green | 23-01 (green run) + 23-07 (badge) | 1, 7 | PORT-03 | — | N/A | manual (post-push) | Badge URL resolves to a green run on `main` at HEAD | ❌ W0 | ⬜ pending |
| Four PORT-04 boundaries present on **both** surfaces, matching shipped reality | 23-06 (page) + 23-07 (README) | 6–7 | PORT-04 | T-23-HONEST | Disclosure describes what **shipped** (PROJECT.md §Key Decisions), not the hypothesis | code review + grep | Grep landing-page source + README for each of the 4 boundaries; diff wording against `.planning/PROJECT.md` §Key Decisions | ❌ W0 | ⬜ pending |
| `pages.yml` deploys `/site` to Pages on merge to `main` | 23-04 | 4 | CICD-02 | T-23-CIPRIV | Elevated (`pages: write` + `id-token: write`) job lives in its **own** workflow, never in PR-triggered `ci.yml`; no `pull_request_target` anywhere | integration (real CI) | Actions tab shows a green `pages.yml` run after a merge to `main`; site loads at `jadrianports.github.io/dexter` | ❌ W0 | ⬜ pending |
| Pages deploy is **really gated** on CI green (not advisory) | 23-04 | 4 | CICD-02 | T-23-CIPRIV | A red `ci.yml` on `main` must **not** publish a page carrying a drifted invite URL | integration | `workflow_run` filtered on `conclusion == success` **and** `head_branch == 'main'`, checking out `head_sha`; prove by observing a red-CI run publishes nothing | ❌ W0 | ⬜ pending |
| `release.yml` publishes multi-arch GHCR image on `v*` tag | 23-04 | 4 | CICD-03 | T-23-SUPPLY | `packages: write` scoped to its own workflow; secrets never baked into layers (T-04-05); `site/`+`node_modules/` excluded via `.dockerignore` | integration (real CI) | Tag `v1.4.0-rc1` → green run; `docker manifest inspect` lists amd64 + arm64 | ❌ W0 | ⬜ pending |
| Image pullable with **zero build step**, logged out | 23-07 (HUMAN-UAT item) | 7 | CICD-03 | — | N/A | **manual-only** (D-17 — GHCR visibility is a UI setting) | `docker logout ghcr.io && docker pull ghcr.io/jadrianports/dexter:v1.4.0` | ❌ W0 | ⬜ pending |
| No second invite-URL constructor / shortener / redirect | 23-03, 23-05, 23-06, 23-07 | 3–7 | (Phase 22 D-07 fence) | T-23-DRIFT | `logic/invite.py::build_invite_url()` remains the sole constructor | code review + grep | Grep repo for `discord.com/oauth2/authorize` outside `logic/invite.py`, tests, and generated `dist/` | ✅ exists | ⬜ pending |
...
**Approval:** approved 2026-07-14 — plans `23-01`…`23-07` authored (`63850e0`) and passed `gsd-plan-checker` on the first pass. Sign-off boxes are execution-time gates, checked as waves land.
