...
**Shipped (code): Phase 26 — Radio Mode & Skip Democracy (2026-07-16)** — 5 plans, 15 tasks. **DJ-01 + DJ-02 validated.** Dexter can now DJ a room indefinitely off the taste brain, and skipping stopped being one user's unilateral call:
- **DJ-01 (Radio Mode):** `/radio start|stop`, a pure `logic/radio.py` refill-gate seam (`should_refill_radio`/`is_already_played`/`has_room_for_refill`), in-memory per-guild armed-state + session played-set on `MusicQueue`, and a keyword-only `radio=True` branch inside the existing `try_auto_queue` brain (lifts the round cap, anchors the seed, hard-filters session repeats after YouTube resolution). D-10 lookahead refills *before* the queue drains (no dead air); D-11 makes radio and loop mutually exclusive; D-07/SC-2 disarm radio at **every** `clear()` teardown, so the disarmed path is structurally unable to refill. The non-radio auto-queue path is byte-identical when disarmed, locked by a regression test.
- **DJ-02 (Skip Democracy):** a pure `logic/skip_vote.py` verdict seam (`floor(n*ratio)+1` clamped, honoring `config.SKIP_VOTE_MAJORITY_RATIO` — deliberately not `n//2+1`), per-track vote set reset on track change, requester-bypass + solo-instant-skip gates, and D-15's single vote-gated choke point: `_do_skip` is called **exactly once**, from inside `_try_skip`, which all three entry points route through. Execution found a *third* skip entry point nobody's planning docs listed — `/seek`'s past-end auto-skip called `_do_skip` directly, a complete vote bypass via `/seek 99:99` — and closed it.
- **Code review caught a Critical that would have defeated the whole requirement:** `/skip` and `/seek` counted skip votes from users **not in the voice channel** (the button gated correctly; those two didn't). Because `decide_skip` deliberately does not filter its numerator by live membership (D-17: a departed voter's vote must keep counting), membership gating is the caller's job — and two of three callers skipped it. Fixed + regression-tested (CR-01), along with 4 warnings: stale radio-era counters at the loop-disarm path (WR-01), a double-skip race (WR-02), paused-track guard inconsistency across entry points (WR-03), and an uncapped free-text radio seed (WR-04). IN-01 (`/radio start` voice-membership) deferred as Info.
- Verifier confirmed 11/11 must-haves against the codebase; suite green at 1175 passed / 129 skipped / 0 failed; `ruff check` clean. 5 live-Discord items (radio cadence feel, multi-listener tally narration, solo-skip regression, post-stop drain, clean-boot command registration) parked in `26-HUMAN-UAT.md` per standing precedent.
...
- **New music muscle** — radio/endless mode (taste-brain-driven) ✅ **shipped (code) Phase 26** + skip-voting/queue democracy ✅ **shipped (code) Phase 26** + crossfade (**spike-gated**: a plan-time prototype must prove `/skip`-mid-fade behaves before full commit, else descope to fast-follow per the standing Descope Rule).
...
---
*Last updated: 2026-07-16 after Phase 26 (Radio Mode & Skip Democracy) — DJ-01 + DJ-02 shipped at the code level; next is Phase 27 (Crossfade, spike-gated)*
