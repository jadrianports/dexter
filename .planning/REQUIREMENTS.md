...
- [ ] **BRAIN-01**: Auto-queue is taste-aware — it incorporates the user's recent taste and `was_skipped` history as negative hints so it stops re-queueing tracks the user skips (Gemini-in-the-loop + SQL, not an ML model).
- [ ] **BRAIN-02**: A discovery command surfaces artist/genre adjacency from listening history via grounded co-occurrence SQL over `song_history`/`user_artist_counts` (multi-user-safe aggregate query; zero hallucination, zero cost).
